...
% The main entry point is diagnose(Observations, Diagnoses).
% Observations is a list of atoms like: [service_returns_500_all_requests, recent_deploy, db_connection_refused, ...].
...
pair_key(-Score, Cause, Ev, Actions, key(-Score, Cause, Ev, Actions)).

unpair_key(key(_NegScore, Cause, Ev, Actions), diag(Cause, Score, Ev, Actions), Score) :-
    % Score is reconstructed by reading it out of the diag after sorting; in our usage,
    % we pass the original Score along before converting to key form.
    % We keep a separate mapping pipeline to retain Score.
    % This helper is unused; see sort_diags/2 for actual mapping.
    Score = 0.

sort_diags(DiagsIn, DiagsOut) :-
    % Sort by descending Score, tie-breaker by standard term order of the rest
    % We convert to key(-Score, Cause, Ev, Actions) so sort/2 gives desired order.
    findall(key(-S, C, Ev, A)-diag(C, S, Ev, A), member(diag(C, S, Ev, A), DiagsIn), Pairs),
    keysort(Pairs, SortedPairs),
    findall(D, member(_Key-D, SortedPairs), DiagsOut).
...
score_cause(Obs, dependency_outage(db), Score, Evidence) :-
    ...
    Score is Base + Extra,
    % Evidence in a predictable order
...
diagnose(Observations, Diagnoses) :-
    ( has(Observations, service_returns_500_all_requests) -> true
    ; % If the blanket 500 signal isn't present, return empty diagnosis
      Diagnoses = [], !, fail
    ),
